  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {step === 0 ? (
          ...
          >
            <Text style={styles.body}>
              Mensu aprende de tus registros y separa lo que escribes de lo que estima.
            </Text>
          </StepShell>
        ) : null}

        {step === 1 ? (
          <StepShell icon="calendar-start" subtitle="Usaremos esta fecha para calcular el día de ciclo inicial." title="¿Cuándo empezó tu última regla?">
        ...
        {step === 2 ? (
          <StepShell icon="tune-variant" subtitle="Puedes corregirlo luego cuando tengas más registros." title="Tu ritmo aproximado">
            <NumberPicker label="Duración del ciclo" onChange={setCycleLength} suffix="días" value={cycleLength} />
            <NumberPicker label="Duración del sangrado" max={10} min={2} onChange={setPeriodLength} suffix="días" value={periodLength} />
          </StepShell>
        ) : null}

        {step === 3 ? (
          <StepShell icon="chart-timeline-variant" subtitle="Esto cambia el nivel de confianza de las predicciones." title="¿Qué tan regular suele ser?">
            <View style={styles.segmentGroup}>
              {REGULARITY.map((item) => (
                ...
                >
                  <Text style={[styles.segmentText, regularity === item.key && styles.segmentTextActive]}>{item.label}</Text>
                </Pressable>
              ))}
            </View>
            <Pressable
        ...
        {step === 4 ? (
          <StepShell icon="star-four-points-outline" subtitle="Mensu no usa esto para diagnosticar ni prometer precisión." title="¿Qué quieres priorizar?">
            <View style={styles.goals}>
              {GOALS.map((item) => (
                <Pressable key={item.key} onPress={() => setGoal(item.key)} style={[styles.goal, goal === item.key && styles.goalActive]}>
        ...
        {step === 5 ? (
          <StepShell icon="bell-outline" subtitle="Podrás cambiarlo en Ajustes. Nada sensible aparece en la notificación." title="Check-ins suaves">
            <View style={styles.reminders}>
              {moments.map((moment) => (
                <Pressable
                  ...
                  onPress={() =>
                    setMoments((current) =>
                      current.map((item) => (item.id === moment.id ? { ...item, enabled: !item.enabled } : item))
                    )
                  }
      ...
      <View style={styles.footer}>
        {step > 0 ? <SoftButton label="Atrás" onPress={() => setStep((current) => current - 1)} variant="ghost" /> : null}
        <SoftButton
