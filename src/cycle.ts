...
export function estimateCycle(settings: AppSettings | null, todayIso = toIsoDate(new Date())): CycleSnapshot {
  ...
  const diff = Math.max(0, daysBetween(lastStart, todayIso));
...
function getPhaseMessage(phase: PhaseKey, nextPeriodInDays: number): string {
  switch (phase) {
    case "menstrual":
      return "Registra flujo, dolor y energía para entender cómo cambia este inicio de ciclo.";
    case "follicular":
      return "Suele ser una etapa de recuperación. Mensu irá comparando esto con tus registros.";
    case "fertile":
...
function buildWeek(todayIso: string, cycleDay: number, periodLength: number, fertileStart: number, fertileEnd: number, cycleLength: number) {
...
export function generateMonthDays(target: Date, settings: AppSettings | null): Array<{ iso: string; day: number; inMonth: boolean; phase: PhaseKey }> {
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    return {
      ...
      phase: estimateCycle(settings, iso).phase
    };
  });
}
...
export function buildPersonalInsights(checkIns: MoodCheckIn[], dailyLogs: DailyLog[]): string[] {
  ...
  if (checkIns.length < 4) {
    return ["Con 4 registros Mensu empezará a mostrar patrones propios, sin asumir causas hormonales."];
  }
  ...
  if (avgStress >= 3.4) insights.push("Parece que el estrés ha sido una señal frecuente en los últimos check-ins.");
  if (avgEnergy <= 2.4) insights.push("Suele aparecer energía baja en tus registros recientes.");
  if (dailyLogs.some((log) => log.symptoms.includes("cólicos"))) insights.push("Los cólicos aparecen en tu diario. Mensu los comparará con próximos ciclos.");

  return insights.length > 0 ? insights : ["Tus registros se ven estables. Mensu seguirá observando cambios por fase."];
}
