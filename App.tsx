...
import { createDefaultNotificationMoments, rescheduleNotificationMoments } from "./src/notifications";
...
export default function App() {
  ...
  const boot = async () => {
    ...
    setData({
      ...loaded,
      notificationMoments: loaded.notificationMoments.length > 0 ? loaded.notificationMoments : createDefaultNotificationMoments()
    });
    setLoading(false);
  };
  ...
  function renderTab() {
    if (activeTab === "calendar") {
      return <CalendarScreen dailyLogs={data.dailyLogs} onOpenCheckIn={openDailyCheckIn} settings={data.settings} />;
    }
    ...
    return (
      <TodayScreen
        ...
        onOpenCheckIn={openDailyCheckIn}
        onOpenQuickCheckIn={() => openQuickCheckIn("now")}
        onOpenSchedule={() => setScheduleVisible(true)}
        snapshot={snapshot}
      />
    );
  }
